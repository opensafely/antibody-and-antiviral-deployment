...
data_stp <- left_join(data_stp_all, data_stp_tpp, by = c("stp_id" = "stp")) %>%
  ...
  mutate(prop = round(n.y/n.y, digits = 0 )) %>%
...
colnames(data_stp_clean) <- c("STP", "Number of patients in TPP", "Total population", "Percentage of population included in TPP (%)",
                        "Number of eligble patients in TPP", "Number of treated patients in TPP", "Percentage treated (%)")

write_csv(data_stp_clean, fs::path(output_dir_rmd, "stp_coverage.csv"))
```
