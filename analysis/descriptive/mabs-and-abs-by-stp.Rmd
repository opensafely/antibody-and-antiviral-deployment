...
eligible_stp_decile <- data_processed_clean %>%
  ...
  select(decile, elig_start, n_stp, n_patients, cum_count_decile = cum_count) #%>%
  #mutate(cum_count = ifelse(cum_count < 8, NA, plyr::round_any(cum_count, 10))) %>%
...
eligible_stp_decile %>%
  ggplot(aes(x = elig_start, y = cum_count_decile, group = decile)) +
...
treated_stp_decile <- data_processed_clean %>%
  ...
  select(decile, treatment_date, n_stp, n_patients, cum_count_decile = cum_count) #%>%
  #mutate(cum_count = ifelse(cum_count < 8, NA, plyr::round_any(cum_count, 10))) %>%

write_csv(treated_stp_decile, fs::path(output_dir_rmd, "figure_1.csv"))

treated_stp_decile <- treated_stp_decile %>%
  mutate(line = ifelse(decile == "0.5", "Median", "Decile"))
  
treated_stp_decile %>%
  ggplot(aes(x = treatment_date, y = cum_count_decile, group = decile)) +
  geom_step(aes(colour = line, linetype = line)) +
  scale_color_manual(values = c("Decile" = "blue", "Median" = "blue")) +
  scale_linetype_manual(values = c("Decile" = "dashed", "Median" = "solid")) +
  theme_classic(base_size = 8) +
...
write_csv(proportion_stp_decile, fs::path(output_dir_rmd, "figure_3.csv"))
