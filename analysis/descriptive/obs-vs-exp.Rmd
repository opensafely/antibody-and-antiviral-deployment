...
The ratio of the observed to expected number of treated patients was calculated for ethnicity, high risk cohort and regional groups. To do this, the expected number of treated patients for each group (adjusted) was calculated by predicting the treatment status for every patient using the above model, and then summing over all patients within each group In this way, we obtain the expected value E, which we can then
compare to the observed value O, highlighting those groups with treatment rates that differed from the expected rate for the population. For example, if O differs more from E than would be expected from chance alone, we can conclude that the group does not perform according to the model benchmark.
