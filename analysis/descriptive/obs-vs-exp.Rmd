...
for (i in 1:length(variables)){
  ...
  res.logist <- glm(formula, data = data_processed_clean, family = binomial(link = 'logit'),)
...
mod.adj <- glm(treated_observed ~ ageband + sex + ethnicity + imd + rural_urban + region_nhs + stp + 
             ...
             data = data_processed_clean)

table.mod.adj <- tidy_wald(mod.adj, exponentiate = TRUE)[-1,]

# Results
results <- data_processed_clean %>%
  mutate(fitted.results = predict(mod.adj, newdata = data_processed_clean, type = 'response'),
         treated_expected = ifelse(fitted.results > 0.9,1,0))
...
# Method
Treatment was modelled using logistic regression. Candidate risk-adjustment variables considered were
...
* STP
* High risk cohort
* Comorbid conditions (including high-risk conditions); autism, care home, dementia, housebound, learning disability, shielded, serious mental illness, sickle cell disease
* Vaccination status


# Results

**Table 1 Count and proportion of eligible patients in OpenSAFELY-TPP between 16th December 2021 23rd February 2022, stratified by treatment status, along with odds ratios from a univariable and multivariable logistic regression model** The multivariable logistic model for treatment had an AUC of `r auc`, a misclassification error rate of `r misClasificError`% and an overall observed to expected ration of `r round(overall$y, digits = 2)`, (95% CI `r `round(overall$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`). Patient counts <8 have been redacted and counts >7 are rounded to the nearest 10; as a result percentages may not add up to 100%."
