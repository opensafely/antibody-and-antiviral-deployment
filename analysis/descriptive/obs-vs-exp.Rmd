...
results <- data_processed_clean %>%
    mutate(fitted.results = predict(mod.adj, newdata = data_processed_clean, type = 'response'),
         treated_expected = ifelse(fitted.results >= 0.3, 1, 0))
...
* STP
* High risk cohort
* Comorbid conditions (including high-risk conditions); autism, care home, dementia, housebound, learning disability, shielded, serious mental illness
* Vaccination status
* Week of treatment
...
The unadjusted and adjusted logistic models for treatment had an overall observed to expected ratio of `r round(overall$y, digits = 2)`, (95% CI `r round(overall$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`) and `r round(overall_unadjusted$y, digits = 2)`, (95% CI `r round(overall_unadjusted$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`), respectively. The multivariable logistic model (with an AUC of `r auc`, a misclassification error rate of `r misClasificError`%) indicated that several variables were associated with being treated (Table 1). After adjustment for all demographic and clinical variables, `r round((1-summary(mod.adj)$dev.expl)*100, digits = 0)` of the variation remained unexplained.
...
**Table 1 Count and proportion of eligible patients in OpenSAFELY-TPP between 16th December 2021 23rd February 2022, stratified by treatment status, along with odds ratios from a univariable and multivariable logistic regression model** Patient counts <8 have been redacted and counts >7 are rounded to the nearest 10; as a result percentages may not add up to 100%."
...
funnel_plot %>%
  ...
  #guides(shape = "none") +
  geom_segment(aes(x = min(dfCI$number.seq), xend =  max(dfCI$number.seq), 
