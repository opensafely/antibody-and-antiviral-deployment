...
## Logistic model - adjusted (without high risk cohort)
...
# Method
Logistic regression was used to model and predict each patients treatment status (either did not receive a nMAB or antiviral as treatment for COVID-19 or received a nMAB or antiviral as treatment for COVID-19), firstly unadjusted, and secondly adjusted for the following demographic and clinical covariates:
...
Unadjusted and adjusted ORs were calculated from the two models, respectively. In addition, the ratio of the observed to expected number of patients receiving treatment was calculated for the ethnicity, high risk cohort and regional groups. To do this, the expected number of patients receiving treatment for each group (adjusted) was calculated by predicting the treatment status for every patient using the above model, and then summing over all patients within each group. In this way, it is possible to obtain the expected value E, which can then be compared to the observed value O, highlighting those groups with treatment rates that differed from the expected rate for the eligible population. Both caterpillar plots and funnel plots were constructed to compare the ratio of the observed to expected number of patients receiving treatment. 


# Results
The multivariable logistic model for treatment status had an overall observed to expected ratio of `r round(overall$y, digits = 2)`, (95% CI `r round(overall$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`), an AUC of `r auc`, and a misclassification error rate of `r misClasificError`%). As shown in Table 1, several variables were associated with treatment status. After adjustment for all demographic and clinical variables, `r round((1-summary(mod.adj)$dev.expl)*100, digits = 0)`% of the variation remained unexplained with the observed to expected (adjusted) ratios of the number of patients receiving treatment varying widely across high risk cohort and region specific groups  (Figures 1-3).
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', out.width = '50%', fig.show = "hold", fig.cap = "**Figure 1. Distribution of propensity scores in patients who did/did not recieved received treatment for COVID-19**" , fig.topcaption=TRUE}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', out.width = '100%', fig.cap = "**Figure 3.  Adjusted funnel plots showing the ratio of the observed to expected number of patients receiving treatment, stratified by (a) ethnicity, (b) high risk cohort and (c) region.** Coloured points represent adjusted ratios. Solid black lines are 95% intervals and black dotted lines are 99.9% intervals, derived using the Poisson distribution. The solid horizontal blue line represent the overall observed to expected ratio of the number of patients receiving treatment", fig.topcaption=TRUE}
