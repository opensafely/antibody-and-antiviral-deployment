...
dfCI <- data.frame(number.seq = seq(0, plyr::round_any(max(funnel_plot$treated_expected, funnel_plot$treated_expected, na.rm = T), 100, ceiling), 1)) %>%
...
funnel_plot %>%
  ...
  #guides(shape = "none") +
  geom_segment(aes(x = 0, xend =  plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling), 
...
funnel_plot %>%
  ...
  theme(legend.position = "bottom",
        legend.title = element_blank()) +
  #guides(shape = "none") +
  geom_segment(aes(x = 0, xend =  plyr::round_any(max(treated_expected, na.rm = T), 100, ceiling), 
                   y = overall$y, yend = overall$y),
               colour = "steelblue2") +
  ggtitle("(b)")
```
