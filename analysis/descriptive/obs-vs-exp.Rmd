...
results <- data_processed_clean %>%
    mutate(fitted.results = predict(mod.adj, newdata = data_processed_clean, type = 'response'),
         treated_expected = ifelse(fitted.results >= 0.4, 1, 0))
...
# Background
As is well known, treatment selection in most observational studies is often influenced by subject characteristics. Eligible patients who receive a nMAB or antiviral as treatment for COVID-19 are therefore likely to be very different to eligible patients who do not receive treatment and baseline characteristics of treated patients will likely differ systematically from those of untreated subjects. For example, patients who are offered treatment will be those who are less likely to be severely ill with COVID-19.

Preliminary results have shown that there is some variation in which eligible patients are receiving treatment. Variation is appearing across high risk cohorts and regions, but it is possible that other factors such as ethnicity are driving these inequalities. Currently numbers are too small to look at results broken down by more than one category (i.e a breakdown of ethnicity by high risk cohort by region would have too few numbers in each group) so there is a need for another method to try to ascertain whether any real variation exists and if so, where it is occurring.
...
# Method
Logistic regression was used to model and predict each patients treatment status (either not treated or treated with a mabs/antiviral), firstly unadjusted, and secondly adjusted for the following demographic and clinical covariates:
...
Unadjusted and adjusted ORs were calculated from the two models. In addition, the ratio of the observed to expected number of treated patients was calculated for the ethnicity, high risk cohort and regional groups. To do this, the expected number of treated patients for each group (adjusted) was calculated by predicting the treatment status for every patient using the above model, and then summing over all patients within each group. In this way, it is possible to obtain the expected value E, which can then be compared to the observed value O, highlighting those groups with treatment rates that differed from the expected rate for the population. Both caterpillar plots and funnel plots were constructed to compare the ratio of the observed to expected number of treated patients. 


# Results
The unadjusted and adjusted logistic models for treatment had an overall observed to expected ratio of `r round(overall_unadjusted$y, digits = 2)`, (95% CI `r round(overall_unadjusted$ymin, digits = 2)` - `r round(overall_unadjusted$ymax, digits = 2)`) and `r round(overall$y, digits = 2)`, (95% CI `r round(overall$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`), respectively. The multivariable logistic model (with an AUC of `r auc`, a misclassification error rate of `r misClasificError`%) indicated that several variables were associated with being treated (Table 1). After adjustment for all demographic and clinical variables, `r round((1-summary(mod.adj)$dev.expl)*100, digits = 0)`% of the variation remained unexplained.


Adjusted ethnic, high risk cohort and regional specific observed to expected ratios varied widely (Figure 1).
...
table1.full <- full_join(table1, table.mod.unadj, by = "term") %>%
  ...
  mutate(Group = factor(Group, 
                        ...
                        labels = c("Age band", "Sex", "Ethnicity", "IMD", "Rurality", "Region", rep("High risk cohort", 10),
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', out.width = '100%', fig.cap = "**Figure 1. Observed to expected ratio of treated patients, stratified by (a) ethnicity, (b) high risk cohort and (c) region.** The squares represent the point estimates, and the areas of the squares are proportional to the observed number of treated patients The vertical lines display 95% confidence intervals. The solid and dashed horizontal blue lines represent the overall observed to expected ratio of treated patients with a 95% CI, respectively.", fig.topcaption=TRUE}
...
results %>% 
  ...
  labs(
    x = "Ethnicity",
    y = "Observed/Expected ratio of treated patients",
    size = "Observed number of treated patients",
    title = "(a)") +
  #geom_hline(yintercept = 1, linetype = "dashed")  +
  geom_hline(yintercept = overall$y, colour = "steelblue2") +
  geom_hline(yintercept = overall$ymin, linetype = "dashed", colour = "steelblue2") +
  geom_hline(yintercept = overall$ymax, linetype = "dashed", colour = "steelblue2") +
  theme(
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', out.width = '100%', fig.cap = "**Figure 2. Unadjusted (left) and adjusted (right) funnel plots showing ratio of the observed to expected number of treated patients, stratified by (a) ethnicity, (b) high risk cohort and (c) region.** Coloured points represent adjusted ratios, respectivly. Solid black lines are 95% intervals and black dotted lines are 99.9% intervals, derived using the Poisson distribution. The solid horizontal blue line represent the overall observed:expected ratio of treated patients", fig.topcaption=TRUE}
...
dfCI_unadj <- data.frame(number.seq = seq(plyr::round_any(min(funnel_plot$treated_expected, na.rm = T), 100, floor), 
                                       plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling), 
                                       1)) %>%
  mutate(number.ll95 = overall_unadjusted$y - 1.96 * sqrt(overall_unadjusted$y/number.seq),
         number.ul95 = overall_unadjusted$y + 1.96 * sqrt(overall_unadjusted$y/number.seq),
         number.ll999 = overall_unadjusted$y - 3.29 * sqrt(overall_unadjusted$y/number.seq),
         number.ul999 = overall_unadjusted$y + 3.29 * sqrt(overall_unadjusted$y/number.seq))

dfCI_adj <- data.frame(number.seq = seq(plyr::round_any(min(funnel_plot$treated_expected, na.rm = T), 100, floor), 
                                    plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling), 
                                    1)) %>%
...
unadj_plot <- funnel_plot %>%
  filter(mod == "unadjusted") %>%
  ggplot(aes(x = treated_expected, y = obs_exp_ratio)) + 
  geom_point(aes(colour = ethnicity), size = 2) +
  geom_line(aes(x = number.seq, y = number.ll95), data = dfCI_unadj) +
  geom_line(aes(x = number.seq, y = number.ul95), data = dfCI_unadj) +
  geom_line(aes(x = number.seq, y = number.ll999), linetype = "dashed", data = dfCI_unadj) +
  geom_line(aes(x = number.seq, y = number.ul999), linetype = "dashed", data = dfCI_unadj) +
  ylim(min(dfCI_adj$number.ll999, dfCI_unadj$number.ll999), max(dfCI_adj$number.ul999, dfCI_unadj$number.ul999)) +
  xlim(plyr::round_any(min(funnel_plot$treated_expected, na.rm = T), 100, floor), 
        plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling)) +
  xlab("Expected number of treated patients") + 
  ylab("Unadjusted observed to expected ratio of treated patients") + 
  theme_bw() +
  theme(legend.position = "bottom",
        legend.title = element_blank()) +
  #guides(shape = "none") +
  geom_segment(aes(x = min(dfCI_unadj$number.seq), xend =  max(dfCI_unadj$number.seq), 
                   y = overall_unadjusted$y, yend = overall_unadjusted$y),
               colour = "steelblue2") +
  ggtitle("(a)")

adj_plot <- funnel_plot %>%
  filter(mod == "adjusted") %>%
  ggplot(aes(x = treated_expected, y = obs_exp_ratio)) + 
  geom_point(aes(colour = ethnicity), size = 2) +
  geom_line(aes(x = number.seq, y = number.ll95), data = dfCI_adj) +
  geom_line(aes(x = number.seq, y = number.ul95), data = dfCI_adj) +
  geom_line(aes(x = number.seq, y = number.ll999), linetype = "dashed", data = dfCI_adj) +
  geom_line(aes(x = number.seq, y = number.ul999), linetype = "dashed", data = dfCI_adj) +
  ylim(min(dfCI_adj$number.ll999, dfCI_unadj$number.ll999), max(dfCI_adj$number.ul999, dfCI_unadj$number.ul999)) +
  xlim(plyr::round_any(min(funnel_plot$treated_expected, na.rm = T), 100, floor), 
        plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling)) +
  xlab("Expected number of treated patients") + 
  ylab("Adjusted observed to expected ratio of treated patients") + 
  theme_bw() +
  theme(legend.position = "bottom",
        legend.title = element_blank()) +
  #guides(shape = "none") +
  geom_segment(aes(x = min(dfCI_adj$number.seq), xend =  max(dfCI_adj$number.seq), 
                   y = overall$y, yend = overall$y),
               colour = "steelblue2") +
  ggtitle(" ")

grid.arrange(unadj_plot, adj_plot, ncol = 2)
...
funnel_plot %>%
  ...
  #xlim(0, plyr::round_any(max(funnel_plot$treated_expected, na.rm = T), 100, ceiling)) +
  #ylim(-11,1) +
  xlab("Expected number of treated patients") + 
...
funnel_plot %>%
  filter(mod == "adjusted") %>%
  ggplot(aes(x = treated_expected, y = obs_exp_ratio)) + 
  geom_point(aes(colour = region_nhs), size = 2) +
  geom_line(aes(x = number.seq, y = number.ll95), data = dfCI) +
  geom_line(aes(x = number.seq, y = number.ul95), data = dfCI) +
  geom_line(aes(x = number.seq, y = number.ll999), linetype = "dashed", data = dfCI) +
  geom_line(aes(x = number.seq, y = number.ul999), linetype = "dashed", data = dfCI) +
...
* Model variables/performance/validity needs more work
...
* Funnel plots might be better for displaying O:E ratios
