...
mod.adj <- gam(treated_observed ~ ageband + sex + ethnicity + imd + rural_urban + region_nhs + stp + 
               ...
               serious_mental_illness_nhsd + vaccination_status + s(week),
...
results <- data_processed_clean %>%
    mutate(fitted.results = predict(mod.adj, newdata = data_processed_clean, type = 'response'),
         treated_expected = ifelse(fitted.results >= 0.35, 1, 0))
...
# Method
The primary outcome was treatment status (either treated with a mabs/antiviral or not treated). Treatment was modelled using logistic regression with candidate predictor variables considered as follows:
...
The ratio of observed to expected number of treated patients was calculated for ethnicity, high risk cohort and regional groups. To do this, the expected number of treated patients for each group (adjusted) was calculated by multiplying the national epilepsy admission rate
per thousand CYP by the total catchment population
for that Trust. The adjusted values for expected admission rates used a similar approach, based on the national
epilepsy admission rate per thousand and the Trust catchment population within each age-sex-IMD group. The
expected total number of admissions for each Trust was
the sum of expected admissions within each age-sex-IMD
group.
...
overdispersion: `r sum(mod.adj$weights * mod.adj$residuals^2)/mod.adj$df.residual`


**Table 1 Count and proportion of eligible patients in OpenSAFELY-TPP between 16th December 2021 23rd February 2022, stratified by treatment status, along with odds ratios from a univariable and multivariable logistic regression model** The multivariable logistic model for treatment had an AUC of `r auc`, a misclassification error rate of `r misClasificError`% and an overall observed to expected ratio of `r round(overall$y, digits = 2)`, (95% CI `r round(overall$ymin, digits = 2)` - `r round(overall$ymax, digits = 2)`). Patient counts <8 have been redacted and counts >7 are rounded to the nearest 10; as a result percentages may not add up to 100%."
...
dfCI <- data.frame(number.seq = seq(plyr::round_any(min(funnel_plot$treated_expected, funnel_plot$treated_expected, na.rm = T), 100, floor), 
                                    ...
                                    1)) %>%
  mutate(number.ll95 = group$y - 1.96 * sqrt(group$y/number.seq),
         number.ul95 = group$y + 1.96 * sqrt(group$y/number.seq),
         number.ll999 = group$y - 3.29 * sqrt(group$y/number.seq),
         number.ul999 = group$y + 3.29 * sqrt(group$y/number.seq))
