...
variables = c("ageband","sex", "ethnicity", "imd", "rural_urban", "region_nhs", "stp", "downs_syndrome", "solid_cancer", 
              ...
              "rare_neurological_conditions", "high_risk_group_combined_count", "autism_nhsd", "care_home_primis", "dementia_nhsd", 
              "housebound_opensafely", "learning_disability_primis", "shielded_primis", "serious_mental_illness_nhsd", 
              "sickle_cell_disease_nhsd", "vaccination_status", "week")

# Model data
data_model <- data_processed_clean %>% 
  select(patient_id, treated_observed, all_of(variables)) %>%
...
table_demo_clinc_breakdown <- data_model %>%
  ...
  tbl_summary(by = treated_observed,
              type = list(high_risk_group_combined_count ~ "continuous",
                          week ~ "continuous")) %>%
  add_overall()
...
mod.adj <- glm(treated_observed ~ ageband + sex + ethnicity + imd + rural_urban + region_nhs + stp + 
               downs_syndrome + solid_cancer + haematological_disease + renal_disease + liver_disease + imid +immunosupression +
               hiv_aids + solid_organ_transplant + rare_neurological_conditions + high_risk_group_combined_count + autism_nhsd +
               care_home_primis + dementia_nhsd + housebound_opensafely + learning_disability_primis + shielded_primis +
               serious_mental_illness_nhsd + vaccination_status + week,
...
results <- data_processed_clean %>%
    mutate(fitted.results = predict(mod.adj, newdata = data_processed_clean, type = 'response'),
         treated_expected = ifelse(fitted.results >= subset(results.table, AUC == max(results.table$AUC))$cutoff, 1, 0))
...
table1.full <- full_join(table1, table.mod.unadj, by = "term") %>%
  ...
  mutate(estimate.x = round(estimate.x, digits = 2),
         ...
         estimate.y = round(estimate.y, digits = 2),
         conf.y = paste("(", round(conf.low.y, digits = 2), " - ", round(conf.high.y, digits = 2), ")", sep = ""),
  ...
  mutate(Group = factor(Group, 
                        levels = c("ageband", "sex", "ethnicity", "imd", "rural_urban", "region_nhs",
                                   ...
                                   "high_risk_group_combined_count", "autism_nhsd", "care_home_primis",
                                   "dementia_nhsd", "learning_disability_primis", "serious_mental_illness_nhsd", 
                                   "housebound_opensafely", "shielded_primis", "sickle_cell_disease_nhsd", "vaccination_status", "week"),
                        labels = c("Age band", "Sex", "Ethnicity", "IMD", "Rurality", "Region", rep("High risk cohort", 10),
                                   "High risk cohort count", rep("Clinical group", 8),
                                   "Vaccination status", "Week")),
         Variable = factor(Variable, 
                           levels = c("12-29", "30-39", "40-49", "50-59", "60-69", "70-79", "80+", 
                                      ...
                                      "Three or more vaccinations", "Unknown", "week"),
                           labels = c("12-29", "30-39", "40-49", "50-59", "60-69", "70-79", "80+", 
                                      "Female", "Male" ,
                                      "White", "Asian or Asian British", "Black or Black British", "Mixed", "Other ethnic groups",
                                      "1 most deprived", "2", "3", "4", "5 least deprived",
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', out.width = '100%', fig.cap = "**Fig 1. Observed to expected (O:E) ratio of treated patients, stratified by (a) ethnicity, (b) high risk cohort and (c) region.** The squares represent the point estimates, and the areas of the squares are proportional to the observed number of treated patients The vertical lines display 95% confidence intervals. The solid and dashed horizontal blue lines represent the overall observed to expected ratio of treated patients with a 95% CI, respectively. An O:E ratio >1.0 indicates that treatment uptake is higher than would be expected, while an O:E ratio <1.0 indicates that treatment uptake is lower than would be expected.", fig.topcaption=TRUE}
