...
coverage_plot <- coverage_plot_data %>%
  ggplot(aes(x = start_date, y = cum_count_redacted, colour = high_risk_cohort, group = high_risk_cohort)) +
  geom_step(size = 1) +
...
$$\\[0.5cm]$$
```{r, elig treatment plot, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '90%', fig.align = 'center', fig.cap = "Figure 2: Cumulative total of patients who received a nMABs or antiviral for treating COVID-19 since 11th December 2021, stratified by high risk cohorts. Note, treated patients can appear in more than one high risk group"}
...
weekly_treatment_proportion_plot <- weekly_treatment_proportion %>%
  ...
  ggplot(aes(x = start_date, y = prop_redacted, colour = treatment_type, group = treatment_type)) +
  geom_line(size = 1) +
  facet_wrap(~high_risk_cohort) +
  theme_classic(base_size = 8) +
  scale_x_date(date_breaks = "2 week", date_labels =  "%d %b %Y") +
  labs(
    x = "",
...
table_elig_treat_redacted <- table_elig_treat_redacted %>%
  mutate(`Number of treated patients, %` = round(Treated/Eligibile*100, digits = 0),
         `Paxlovid, n (%)` = paste(Paxlovid, " (", round(Paxlovid/Treated*100, digits = 0), ")", sep = ""),
         `Remdesivir, n (%)` = paste(Remdesivir, " (", round(Remdesivir/Treated*100, digits = 0), ")", sep = ""),
         `Casirivimab, n (%)` = paste(Casirivimab, " (", round(Casirivimab/Treated*100, digits = 0), ")", sep = ""),
         `Molnupiravir, n (%)` = paste(Molnupiravir, " (", round(Molnupiravir/Treated*100, digits = 0), ")", sep = ""),
         `Sotrovimab, n (%)` = paste(Sotrovimab, " (", round(Sotrovimab/Treated*100, digits = 0), ")", sep = "")) %>%
  ...
  mutate(`Casirivimab, n (%)` = ifelse(`Casirivimab, n (%)` == "NA (NA)", "<5", `Casirivimab, n (%)`),
         `Paxlovid, n (%)` = ifelse(`Paxlovid, n (%)` == "NA (NA)", "<5", `Paxlovid, n (%)`),
         `Remdesivir, n (%)` = ifelse(`Remdesivir, n (%)` == "NA (NA)", "<5", `Remdesivir, n (%)`))

colnames(table_elig_treat_redacted) <- c("High risk cohort", "Count", "Count", "%", "Count (%)", "Count (%)", "Count (%)", 
                                         "Count (%)", "Count (%)")

table_elig_treat_redacted %>%
  kable(caption = "Table 1: Proportion of eligble patients who have recieved treatment for COVID-19 since 11th December 2021, broken down by high risk cohort and treatment type", 
        row.names = FALSE, 
        align = c("l", rep("c", 8)),
        booktabs=TRUE) %>%
  kable_styling(position = "center", full_width = F, bootstrap_options = "striped")  %>%
  column_spec(4, background = "lightgrey")  %>%
  add_header_above(., c(" ", "Eligible", "All" = 2, "Paxlovid  " = 1, "Sotrovimab" = 1, "Remdesivir" = 1,
                        "Molnupiravir" = 1, "Casirivimab" = 1)) %>%
  add_header_above(., c(" ", "", "Treated" = 7))
```
...
table_demo_clinc_breakdown_redacted <- read_csv(fs::path(input_dir_os, "table_demo_clinc_breakdown_redacted.csv")) %>%
  mutate(Group = factor(Group, 
                        ...
                        labels = c("Age band", "Sex", "Ethnicity", "IMD", "Rurality", "Region", "Autism", "Care home", "Dementia",
                                   "Learning disability ", "Serious mental illness", "Housebound", "CEV", "Vaccination status")),
  ...
  mutate(Treated_perc = round(Treated/All*100, digits = 0),
         Paxlovid = paste(Paxlovid, " (", round(Paxlovid/Treated*100, digits = 0), ")", sep = ""),
         Sotrovimab = paste(Sotrovimab, " (", round(Sotrovimab/Treated*100, digits = 0), ")", sep = ""),
         Remdesivir = paste(Remedesivir, " (", round(Remedesivir/Treated*100, digits = 0), ")", sep = ""),
         Molnupiravir = paste(Molnupiravir, " (", round(Molnupiravir/Treated*100, digits = 0), ")", sep = ""),
         Casirivimab = paste(Casirivimab, " (", round(Casirivimab/Treated*100, digits = 0), ")", sep = "")) %>%
  select(Group, Variable, All, Treated, Treated_perc, Paxlovid, Sotrovimab, Remdesivir, Molnupiravir, Casirivimab) %>%  
  mutate(Casirivimab = ifelse(Casirivimab == "NA (NA)", "<5", Casirivimab),
         Paxlovid = ifelse(Paxlovid == "NA (NA)", "<5", Paxlovid),
         Remdesivir = ifelse(Remdesivir == "NA (NA)", "<5", Remdesivir),
         Sotrovimab = ifelse(Sotrovimab == "NA (NA)", "<5", Sotrovimab),
         Molnupiravir = ifelse(Molnupiravir == "NA (NA)", "<5", Molnupiravir))

colnames(table_demo_clinc_breakdown_redacted) <- c("Group", "Variable", "Count", "Count", "%",
                                                   "Count (%)", "Count (%)", "Count (%)", "Count (%)", "Count (%)")
table_demo_clinc_breakdown_redacted %>%
  kable(caption = "Table 2: Count and proportion of eligible patients who have received treatment for COVID-19, broken down by demographic and clinical categories and by and treatment type", 
        row.names = FALSE, 
        align = c("l", rep("c", 9)),
        booktabs=TRUE) %>%
  kable_styling(position = "center", full_width = F, bootstrap_options = "striped")  %>%
...
flowchart_non_elig_plot <- flowchart_non_elig[-1,] %>%
mutate(criteria2 = c("No positive SARS-CoV-2 test record",                       
                     ...
                     "Treated with paxlovid 5 days post symptom onset",           
                     ...
                     "Treated with sotrovimab 5 days post symptom onset",
                     "Treated with sotrovimab aged under 12",  
                     "Treated with sotrovimab weight under 40kg", 
                     "Treated with remdesivir 7 days post symptom onset",
                     "Treated with remdesivir aged under 12",  
                     "Treated with remdesivir weight under 40kg", 
                     "Treated with molnupiravir 7 days post symptom onset",
                     "Treated with molnupiravir and pregnant")) %>%
  filter(!is.na(n)) %>%
  filter(criteria != "c_no_high_risk_group_nhsd",
         criteria != "c_no_high_risk_group_match") %>%
  arrange(desc(n))
