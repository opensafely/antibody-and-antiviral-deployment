...
- PF-07321332 (nirmatrelvir) plus ritonavir: `r format(report_stats$treated_paxlovid, big.mark = ",", scientific = FALSE)`;
...
weekly_treatment_proportion <- read_csv(here::here(input_dir_os, "table_prop_treated_redacted.csv")) %>%
  ...
  mutate(treatment_type = case_when(
    ...
    treatment_type == "Paxlovid" ~ "PF-07321332 (nirmatrelvir) plus ritonavir", 
...
weekly_treatment_proportion_plot <- weekly_treatment_proportion %>%
  mutate(treatment_type = factor(treatment_type, levels = c("All", "PF-07321332 (nirmatrelvir) plus ritonavir", "Sotrovimab", "Remdesivir", 
...
table_elig_treat_redacted %>%
  ...
  add_header_above(., c(" ", "Eligible", "All" = 2, "PF-07321332 (nirmatrelvir) plus ritonavir  " = 2, "Sotrovimab" = 2, "Remdesivir" = 2,
...
flowchart_non_elig_plot <- flowchart_non_elig[-c(1,8),] %>%
  mutate(criteria2 = c("No positive SARS-CoV-2 test record", 
                       ...
                       "Treated with PF-07321332 (nirmatrelvir) plus ritonavir outside of treatment eligibility window",           
                       "Treated with PF-07321332 (nirmatrelvir) plus ritonavir and in renal or liver high risk cohort",
                       "Treated with PF-07321332 (nirmatrelvir) plus ritonavir aged under 18",                       
                       "Treated with PF-07321332 (nirmatrelvir) plus ritonavir and pregnant",
