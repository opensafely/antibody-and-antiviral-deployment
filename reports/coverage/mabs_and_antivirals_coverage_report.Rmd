...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "**Table 1 Count and proportion of potentially eligible patients in OpenSAFELY-TPP who have received treatment for COVID-19 between 11th December 2021 - 28th April 2022, broken down by high risk cohort and treatment type.** Patient counts of 0-7 are shown as <8 with remaining counts rounded to the nearest 10; as a result percentages may not add up to 100%", fig.topcaption=TRUE}
...
weekly_treatment_proportion_plot <- weekly_treatment_proportion %>%
  ...
  theme_classic(base_size = 8) +
  scale_x_date(limit = c(ymd("20211211"), ymd("20220502")),
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.width=10, out.width = '100%', fig.cap = "***Table 2 Count and proportion of potentially eligible patients in OpenSAFELY-TPP who have received treatment for COVID-19 between 11th December 2021 and 28th April 2022, broken down by demographic and clinical categories and by treatment type.** Patient counts of 0-7 are shown as <8 with remaining counts rounded to the nearest 10; as a result percentages may not add up to 100%.", fig.topcaption=TRUE}
...
# maximum over time in first decile
proportion_stp_decile %>%
  filter(decile == 0.1) %>%
  summarise(max(decile_value))

# maximum over time in ninth decile
proportion_stp_decile %>%
  filter(decile == 0.9) %>%
  summarise(max(decile_value))
...
ggsave(
  here::here(output_dir_rmd, "figures", "figure_proportion_stp_decile.jpg"),
  proportion_stp_decile,
  units = "cm", width = 30, height = 15
)
