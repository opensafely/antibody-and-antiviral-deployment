...
time_groups <- data_time_between %>%
  mutate(high_risk_cohort = coalesce(high_risk_group_elig,high_risk_cohort),
         Count = coalesce(n,Count)) %>%
...
time_groups %>%
  arrange(high_risk_cohort, tb_postest_treat) %>%
  ggplot(aes(tb_postest_treat)) +
  geom_col(aes(x = tb_postest_treat, y = Count, fill = high_risk_cohort, group = high_risk_cohort), position = "dodge") + 
  theme_bw(base_size = 8) +
...
colnames(table_outcomes) <- c("Outcome", 
                                                   ...
                                                   "Count",	"%")
table_outcomes %>%
  kable(caption = "Table 3: Count and proportion of patients with COVID-19 outcomes. Occurances of hospitalisation and death are included if they occure one day or more after a patient is eligible or recieves treatemnt (start date). Positive SARS-CoV-2 tests are included if they occure 30 days or more since each patients start date.", 
        row.names = FALSE, 
        align = c("l", rep("c", 4)),
        booktabs=TRUE, 
        table.attr = "style='width:30%;'") %>%
